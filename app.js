...
app.get('/d',(req,res)=>{
   ...
   WHERE e.emp_no = '${empid}' 
    ...
    // app.post('/done', (req, res)=>{
    //     const{fname,mname,lname,email,password,parental,department,empid}=req.body
    //     console.log(empid)
    //     const user='user';
    //     const saltRounds=10;
    //     bcrypt.hash(password, saltRounds, function(err, hash) {
    ...
    //     let query2=`select user_id from asset.dbo.Users where user_id='${empid}'`;
         
    //     let query3=`select * from asset.dbo.Employees where dept_work='${department}' and emp_no='${empid}'`
    
    //     let query4=`select count(*) from asset.dbo.Users where user_name='${email}' `
    
    //     let query5=`select * from asset.dbo.Users where email='${email}'`
    
    //     let query6=`select * from asset.dbo.Employee_master where  official_email='${email}' and emp_id= '${empid}'`
    ...
    //                      if(result3.recordset==""){
    
    //                         res.send({
    //                             code: "Please provide correct department name",
    //                             response: "Please provide correct department name"
    
    //                             })
    //                     }
    //                     else if(result3.recordset!=0){
    
    //                         mssql.query(query6,(err,result6)=>{
    
    //                                 if(err) throw err;
    
    //                             if(result6.recordset==""){
    //                                console.log(2)
    //                                 res.send({
    //                                     code: "Please provide correct email_id",
    //                                     response: "Please provide correct email_id"
    
    //                                     }) 
    //                                 }
    //                             else if(result6.recordset!=0)  
    //                             {
    //                                 console.log(1)
    //                                     mssql.query(query5,(err,result5)=>{
    //                                         if(result5.recordset!=0)
    //                                         {
    //                                             console.log('8')
    //                                             res.send({
    //                                                 code: "User_email_id_exist",
    //                                                 response: "User_email_id_exist"
                                
    //                                                 }) 
    
    //                                         }
    //                                         else if(result5.recordset=="")
    //                                         {
    //                                             console.log(6)
    ...
    //                                                 if(result4.recordset!=0)
    //                                                 {
    //                                                         console.log(10)
    //                                                     res.send({
    //                                                         code: "User_name_exist",
    //                                                         response: "User_name_exist"
                                        
    //                                                         }) 
    //                                                 }
    //                                                 else if(result4.recordset=="")
    //                                                 {
    //                                                       console.log(20)
...
app.post('/assetupload', upload.single('uploadFile'), function (req, res) {
  ...
  function insertDataToAsDatabase2(Asdata2) {
    ...
    sql.query(`INSERT INTO tags (tag_type,tag_uuid) VALUES ( '${tagt}','${Asdata2.tag_uuid}')`, function (err) {
      if (err) {
        console.error('Error inserting data into the database1: ', err);
      }
    });
    ...
    try {
      fs.createReadStream(filePath)
        ...
        .on('end', function () {
          ...
          if(results.length > 0) {
          for (let i = 0; i < results.length; i++) {
            // if (results[i].email.charCodeAt(0) !== 32 && results[i].email.charCodeAt(0) !== null) {
            ...
            // }
            // else {
            //   console.log("Empty space was identified!!");
            // }
          ...
          if (responseStr)
            ...
            // console.log(results);
            for (let i = 0; i < results.length; i++) {
              // insertDataToDatabase1(results[i]);
              // insertDataToDatabase2(results[i]);
              console.log('data inserted in db');
            }
          }
        })
  ...
  function insertDataToDatabase1(data1) {
    console.log(data1);
    sql.query(`INSERT INTO Users (user_id,user_name,email,password,user_type,Parent_org) VALUES ('${data1.user_id}', '${data1.first_name} ${data1.middle_name} ${data1.last_name}', '${data1.email}','${data1.password}','${data1.user_type}','${data1.Parent_org}')`, function (err) {
      if (err) {
        console.error('Error inserting data into the database1: ', err);
      }
    });
